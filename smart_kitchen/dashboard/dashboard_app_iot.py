...
def on_connect(client, userdata, flags, rc):
    if rc == 0:
        ...
        print("✅ MQTT connected for dashboard (IoT Core).")
...
def start_mqtt_thread():
    if st.session_state.mqtt_client is None:
        client = mqtt.Client(client_id='smart-kitchen-dashboard')
...
if section == "Live Sensors":
    ...
    if st.session_state.mqtt_connected:
        st.success("🔴 Live MQTT feed active (IoT Core)!")
    else:
    ...
    if st.session_state.mqtt_connected:
        st.success("Receiving live alerts via MQTT (IoT Core)!")
...
# Footer
st.markdown("---")
st.caption(f"Last updated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | MQTT Endpoint: {MQTT_ENDPOINT}:{MQTT_PORT}")